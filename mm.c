...
{                                         // no guarantee all given pages are mapped
   ...
   *      [addr to addr + pgnum*PAGING_PAGESZ in page table caller->mm->pgd[]
   */
 ...
 struct vm_rg_struct *ret_rg)
{
    ...
    for (pgit = 0; pgit < pgnum; pgit++) {
        ...
        if (init_pte(pte, 1, fpn, 0, 0, 0, 0) != 0) {
            // Invalid setting, clean up and return failure
            free(fpit);
            return -1;
        }
 ...
 */
int vm_map_ram(struct pcb_t *caller, int astart, 
int aend, int mapstart, int incpgnum, struct vm_rg_struct *ret_rg)
{
